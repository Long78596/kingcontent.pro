...
import {
  ...
  actionGetDouyinFollowingChannels,
  actionGetDouyinVideosDouyinInCollections,
...
const LeftPart = (props) => {
  ...
  useEffect(() => {
    if (!collections) {
      ...
      dispatch(actionGetDouyinVideosDouyinInCollections());
    }
  }, [collections]);
  ...
  const onFilterChannel = (e) => {
    ...
    const filtered = followingChannels.filter((channel) =>
        ...
        {isShowChannels ? (
          <Fragment>
            ...
            {filteredChannels?.length > 0 ? (
              <div className="list_channel max-h-screen overflow-auto">
              ...
              <h3 className="font-bold px-2 text-center mt-5">
                Bạn chưa theo dõi kênh Douyin nào Hoặc không tìm thấy kênh
              </h3>
            )}
          </Fragment>
        ) : null}
        ...
        {isShowCollections ? (
          <div className="collectionsContainer">
            ...
            {collections?.length > 0 ? (
              <div className="list_collection max-h-screen overflow-auto">
              ...
              <h3 className="font-bold px-2 text-center mt-5">
                Bạn chưa tạo bộ sưu tập Douyin nào
              </h3>
            )}
          </div>
        ) : (
