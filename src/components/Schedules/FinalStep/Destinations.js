...
const Destinations = (props) => {
  ...
  const onClickRemoveAllDestinations = () => {
    setSelectedDestinations([]);
  };

  const onFilterDestinations = (value, type) => {
    if (value) {
      switch (type) {
        case 'fanpage':
  ...
  const onClickShowFanpages = () => {
    setIsShowFanpages((state) => !state);
  };

  const onClickShowGroups = () => {
      ...
      {listDestinations && listDestinations.length === 0 ? (
        ...
        <Fragment>
          ...
          <div className="listFanpages m-2 overflow-hidden">
            <div className="title p-3 font-bold text-base uppercase mb-2 cursor-pointer border rounded-md flex items-center gap-2">
              <h4 className="w-full" onClick={() => onClickShowFanpages()}>
                CHỌN NƠI ĐĂNG BÀI
              </h4>
              {selectedDestinations.length > 0 && (
                <p className="text-sm normal-case whitespace-nowrap gap-2 ml-auto">
                  (Đã chọn {selectedDestinations.length}{' '}
                  <label
                    className="text-red-500 cursor-pointer"
                    onClick={onClickRemoveAllDestinations}
                  >
                    Xoá
                  </label>
                  )
