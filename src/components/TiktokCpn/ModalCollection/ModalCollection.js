  ...
  const onSave = useCallback(async () => {
    switch (modalCollectionType) {
      ...
      case 'addVideo':
              ...
              buttons: [
                {
                  ...
                  onClick: async () => {
                    ...
                    setIsLoading(true);
                    await dispatch(
                      actionSaveVideoToCollection(selectedCollection?.value, '', videos)
                    );
        ...
        // const videos = chosenVideos.map((video) => ({
        //   feed_id: video.post_id,
        //   feed_name: video.feed_username,
        //   feed_username: video.thumbnail,
        // }));
        const videos = chosenVideos.map((video) => ({
          post_id: video.video_id,
          ...video,
              ...
              buttons: [
                {
                  ...
                  onClick: async () => {
                    setIsLoading(true);
                    await dispatch(
                      actionSaveVideoToCollection(0, collectionName, videos)
                    );
                    dispatch(actionUpdateChosenVideos([]));
                    setIsLoading(false);
